...
// 禁止在 src 中直接裸调 ctx.webServer.register（必须经 registerRoute / ctx.effect 登记，
...
for (const f of files) {
  ...
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i]
    // 裸调用：ctx.webServer.register(...) 且不是 registerRoute helper 内部那行
    if (line.includes('ctx.webServer.register(') && !line.trim().startsWith('//')) {
